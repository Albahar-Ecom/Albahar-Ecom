...
const CouponCode = props => {
    ...
    const onError = errorTarget => {
        ...
        if (setIsCartUpdating)
            setIsCartUpdating(false)
        hideFogLoading()
    }

    const onCompleted = resultContent => {
        if (resultContent && resultContent.applyCouponToCart && resultContent.applyCouponToCart.cart)
            toggleMessages([{ type: 'success', message: Identify.__('You used coupon code.'), auto_dismiss: true }])
    ...
    return (
        <div className="coupon-code">
            ...
            >
                <div className="coupon-code-title-label">{Identify.__('Add a Coupon Code')}</div>
                <div className="coupon-code-title-arrow">{isCouponOpen ? <ArrowUp /> : <ArrowDown />}</div>
            </div>
            <div
                className={`coupon-code-area  ${isCouponOpen ? 'coupon-open' : 'coupon-close'}`}
            >
                <input
                    key={value ? value : 'coupon_field'}
                    className="coupon_field"
                    type="text"
                    placeholder={value ? value : Identify.__('Enter discount code')}
                    defaultValue={value}
                ...
                {(applyingCoupon || !value) ? (
                    ...
                    />
                        ...
                        <Colorbtn
                            className={`submit-coupon ${Identify.isRtl() && 'submit-coupon-rtl'} ${applyingCoupon && 'loading'}`}
