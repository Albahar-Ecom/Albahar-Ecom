...
const CartItem = props => {
    const { currencyCode, item, handleLink, miniOrMobile, setIsCartUpdating, setActiveEditItem } = props
    ...
    if (item.configurable_options && item.configurable_options.length) {
        item.configurable_options.map((configurable_option, cfo_idx) => {
            optionText.push(
                <div key={cfo_idx}>
                    <span>{ReactHTMLParse(configurable_option.option_label)}</span> : {ReactHTMLParse(configurable_option.value_label)}
                </div>
            )
        })
    }
    if (item.bundle_options && item.bundle_options.length) {
        item.bundle_options.map((bundle_option, cfo_idx) => {
            optionText.push(
                <div key={cfo_idx}>
                    <span>{ReactHTMLParse(bundle_option.label)}</span> : {ReactHTMLParse(bundle_option.values[0].label)} ({bundle_option.values[0].quantity})
                </div>
            )
        })
    }
    ...
    else if (item.virtual_customizable_options && item.virtual_customizable_options.length)
        customizable_options = item.virtual_customizable_options
    else if (item.downloadable_customizable_options && item.downloadable_customizable_options.length)
        customizable_options = item.downloadable_customizable_options
    else if (item.bundle_customizable_options && item.bundle_customizable_options.length)
            ...
            optionText.push(
                <div key={cfo_idx}>
                    <span>{ReactHTMLParse(customizable_option.label)}</span> : {
                        customizable_option.values[0].label ? customizable_option.values[0].label :
    ...
    let rowTotal = 0
    if (item.prices && item.prices.row_total && item.prices.row_total_including_tax) {
        rowTotal = tax_cart_display_price == 1 ? item.prices.row_total.value : item.prices.row_total_including_tax.value
    }
    const subtotal = (rowTotal) ? <Price
    ...
    const cartItemImage = (
        ...
        >
            <Image
                src={(product.small_image && product.small_image.url) ? product.small_image.url : logoUrl()}
                ...
                <div className="cart-item-detail">
                    ...
                    <div className="item-qty">
                        ...
                        <input
                            ...
                            onBlur={(event) => {
                                if (parseInt(event.target.value, 10) !== parseInt(item.quantity, 10)) {
                                    handleUpdateItemQuantity(parseInt(event.target.value, 10))
                                }
                            }
                            }
                        />
    ...
    return (
        <tr className="simicart-siminia-cart-item" >
            ...
            <td className="td-item-price">
                ...
                <Price currencyCode={currencyCode} value={rowTotal/(item.quantity ? item.quantity : 1)} />
            </td>
            <td className="td-item-qty">
                <div className="item-qty">
                    <div className="minus-quantity" role="presentation" onClick={() => reduceVal(quantityInputId)}>-</div>
                    <input defaultValue={item.quantity} type="number" id={quantityInputId} />
