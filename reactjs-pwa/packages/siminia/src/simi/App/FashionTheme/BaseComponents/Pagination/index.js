...
class Pagination extends React.Component {
    ...
    renderPageNumber = (total) => {
        ...
        const renderPageNumbers = pageNumbers.map(number => {
            ...
            if (Identify.isRtl()) {
                active.marginRight = 0
                active.marginLeft = 10
            }
            return (
                <li
                    role="presentation"
        ...
        const styleNextPagination = { cursor: 'pointer', fontSize: 14, marginRight: 10, display: 'flex', alignItems: 'center' }
        if (Identify.isRtl()) {
            styleNextPagination.marginRight = 0
            styleNextPagination.marginLeft = 10
        }
        const pagesSelection = (total > 1) ? (
            <ul id="page-numbers" classes={classes["page-numbers"]} style={{
                ...
                <li role="presentation" className={classes["icon-page-number"]} style={styleNextPagination} onClick={() => this.handleChangePage(false)}>
                    {Identify.isRtl() ? <i className="icon icon-chevron-right"></i> : <i className="icon icon-chevron-left"></i>}
                </li>
                {renderPageNumbers}
                <li role="presentation" className={classes["icon-page-number"]} style={{ cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center' }} onClick={() => this.handleChangePage(true)}>
                    {Identify.isRtl() ? <i className="icon icon-chevron-left"></i> : <i className="icon icon-chevron-right"></i>}
                </li>
            </ul>
        ) : '';
