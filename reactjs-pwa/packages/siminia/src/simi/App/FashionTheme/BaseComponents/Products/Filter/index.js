...
import Dropdownplus from 'src/simi/App/FashionTheme/BaseComponents/Dropdownplus';
...
const Filter = (props) => {
    const { data, filterData, history, location,
        total_count, maxPrice, minPrice } = props;
    ...
    const renderClearButton = () => {
        return <div className="clear-filter">
            <div role="presentation" onClick={clearFilter} className="action-clear">{Identify.__('Clear all')}</div>
        </div>
    }
    ...
    const renderRangePrices = memoize(filterData => {
        ...
        return <RangeSlider priceFrom={Number(from)} priceTo={Number(to)}
            clickedFilter={clickedFilter} leftPrice={Number(left)} rightPrice={Number(right)}
        ...
        if (item) {
            if (item.filter_items !== null) {
                options = item.filter_items.map(function (optionItem) {
                    ...
                    return (
                        <div role='presentation' className="filter-item" key={item.request_var + '_' + optionItem.value_string}>
                            <input type="checkbox" defaultChecked={filtersToApply[item.request_var] && filtersToApply[item.request_var].includes(optionItem.value_string)} onChange={() => clickedFilter(item.request_var, optionItem.value_string)} />
                            <span className="filter-item-text">{Identify.__(name)}</span>
                        </div>
                    );
                }, this, item);
            }
        ...
        if (maxPrice && minPrice && minPrice < maxPrice && total_count > 0) {
            rowFilterAttributes.push(
                <Dropdownplus
                    ...
                    <div id={`filter-option-items-price`} className="filter-option-items">
                ...
                if (filterOptions.length > 0) {
                    ...
                    rowFilterAttributes.push(
                        <Dropdownplus
                            classes={dropDownPlusClasses}
                            key={Identify.randomString(5)}
                            id={Identify.randomString(5)}
                            title={Identify.__(item.name)}
                            expanded={filtersToApply[item.request_var] ? true : false} >
                            <div id={`filter-option-items-${item.request_var}`} className="filter-option-items">
                                {filterOptions}
                            </div>
                        </Dropdownplus>
                    )
    ...
    return <div className='filter-products'>
        {filterData && renderClearButton()}
        {renderFilterItems()}
        <div role="presentation" className="apply-filter" onClick={applyFilter} >
            <div className="apply">{Identify.__('Apply')}</div>
        </div>
    </div>
};
