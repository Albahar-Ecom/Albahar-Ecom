...
const LoadMore = (props) => {
    ...
    return (
        <div className="load-more" role="presentation" onClick={() => handleSetPage(currentPage + 1)} >
            <div className="btn-load-more" >
                {loading ? <Loading divStyle={{ marginTop: '-25px' }} loadingStyle={{ fill: 'white' }} /> : Identify.__('Load More')}
            </div>
        </div>
    )
}
