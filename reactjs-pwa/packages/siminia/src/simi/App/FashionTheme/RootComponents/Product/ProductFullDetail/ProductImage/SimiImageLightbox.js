...
class SimiImageLightbox extends Component {
    ...
    renderImages() {
        ...
        return images.map((image, index) => {
            ...
            return <img key={index} src={image} alt={image} style={style} />
        ...
        return (
            <div id="myModal" className={`lighbox-modal modal ${open ? 'active' : ''}`}>
                <div className="bar-icon">
                    <span className="zoom-in cursor" onClick={() => this.handleZoom('in')}><i className="icon-zoom-in"></i></span>
                    <span className="zoom-out cursor" onClick={() => this.handleZoom('out')}><i className="icon-zoom-out"></i></span>
