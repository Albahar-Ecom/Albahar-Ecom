...
import ImageLightbox from "./SimiImageLightbox";
...
class ProductImage extends React.Component {
    ...
    constructor(props) {
        ...
        this.showStatus = this.props.hasOwnProperty('showStatus') ? this.props.showStatus : true;
    ...
    renderImageLightboxBlock = () => {
        ...
        images = images.map((item) => {
            return item.file ? resourceUrl(item.file, { type: 'image-product', width: 640 }) : transparentPlaceholder
        });
        return (
    ...
    renderImage() {
        ...
        return this.images.map(function (item) {
            const src = item.file ? resourceUrl(item.file, { type: 'image-product', width: 640 }) : transparentPlaceholder;
            return (
                <div key={Identify.randomString(5)} style={{ cursor: 'pointer', backgroundColor: '#ffffff', minHeight: width }} className="carousel-image-container">
    ...
    renderJs = () => {
        $(document).ready(function () {
            ...
            carousel.height(mediaWidth * 1.27);
            $('.carousel.carousel-slider img').css({ 'max-height': mediaWidth * 1.27 }); */
            /* setTimeout(() => {
                const mediaWidth = carousel.width();
                carousel.height(mediaWidth * 1.27);
                $('.carousel.carousel-slider img').css({ 'max-height': mediaWidth * 1.27 + 'px' });
            }, 200) */
        });
    }
    ...
    render() {
        ...
        return (
            <div className="product-detail-carousel">
                {!hasStock && <span className="out-of-stock-img">{Identify.__("Out of stock")}</span>}
                <Carousel
                    key={(images && images[0] && images[0].file) ? images[0].file : Identify.randomString(5)}
                    showArrows={this.showArrows}
                    showThumbs={images && images instanceof Array && images.length > 1 ? this.showThumbs : false}
