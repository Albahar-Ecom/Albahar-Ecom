...
class BundleOption extends OptionBase {
    ...
    renderAttribute = (type, obj, id, labelReq) => {
        ...
        return (
            <div key={id} className="option-select">
                <div className="option-title" style={{ fontSize: '14px', lineHeight: '19px', color: '#000' }}>
                    <span>{obj.title} <span style={{ marginLeft: '5px', color: 'red' }}>{labelReq}</span></span>
                </div>
                <div className="option-content" id="bundle-options">
                    ...
                    {!hidden &&
                        <div className={`bundle-option-qty ${type}`} data-name={id} style={{
                            ...
                            <span style={{ fontSize: 16, lineHeight: '22px', fontWeight: 600 }}>{Identify.__('Quantity')}</span>
                            <Qty
                                dataId={id}
                                key={selected[id] ? selected[id] : id}
                                className={`option-qty option-qty option-qty-${id}`}
                                value={defaultQty}
                                inputStyle={{ margin: '0 25px', borderRadius: 0, border: 'solid #727272 1px', maxWidth: 82, color: customQty ? 'black' : '#aeaeae', height: 40 }}
    ...
    renderContentAttribute = (ObjOptions, type = 'checkbox', id) => {
        if (type === 'select') {
    ...
    render() {
        ...
        return (
            <div className="bundle-option-container">
                ...
                <div className="bundle-title-customize">{Identify.__('Customize') + ' ' + props.parent.props.product.name}</div>
                <div className={`bundle-show-options`} /* style={{display : 'none'}} */>
                    {this.renderOptions()}
                    {parseInt(priceInclTax, 10) ?
                        <div className="customized-area">
                            ...
                            <div
                                ...
                                style={{ display: (priceInclTax && priceExclTax) ? 'block' : 'none', paddingBottom: 0, borderBottom: 0, marginTop: 10 }}>
