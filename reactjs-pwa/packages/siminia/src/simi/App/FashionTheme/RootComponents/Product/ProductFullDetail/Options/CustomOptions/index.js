...
class CustomOptions extends OptionBase {
    renderOptions = () => {
        if (this.data instanceof Object && this.data.hasOwnProperty('custom_options')) {
            ...
            const optionsHtml = options.map(function (item) {
                const labelRequired = mainClass.renderLabelRequired(item.required ? 1 : 0);
                if (item.required) {
    ...
    renderContentOption = (ObjOptions, type) => {
        ...
        if (type === 'field') {
            const { field_value } = ObjOptions;
            return <TextField id={id} parent={this} max_characters={field_value.max_characters} />
        }
        if (type === 'area') {
