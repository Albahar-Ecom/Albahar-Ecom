...
import { GET_ORDER_LIST, RE_ORDER_ITEMS } from './OrderPage.gql';
...
const OrderList = props => {
    ...
    if (data && data.items instanceof Array && data.items.length > 0) {
        listOrder = data.items.sort((a, b) => b.id - a.id);
    }

    const noOrders = !data || !data.hasOwnProperty('items') || data.items.length === 0
    return (
        <div className={`customer-recent-orders ${noOrders ? 'no-order' : 'has-order'}`}>
            {noOrders
                ? (
