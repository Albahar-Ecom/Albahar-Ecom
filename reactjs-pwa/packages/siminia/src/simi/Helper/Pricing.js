...
export const formatPrice = (price, currencyCode = null) => {
    ...
    if (storeConfig && storeConfig.simiStoreConfig && storeConfig.simiStoreConfig.config && storeConfig.simiStoreConfig.config.base) {
        ...
        if (currencies instanceof Array && currencies.length && currencyCode !== base.currency_code && currencies.find(({ value }) => currencyCode === value)) {
            const foundSymbol = currencies.find(({ value }) => currencyCode === value);
        ...
        currency_symbol = <span className={`simi-currency-symbol ${currency_position}`}>{currency_symbol}</span>
        return (
            <span className="price manually-formated-price">
                {negativeVal && '-'}
                {currencyBefore && currency_symbol}
                <span className="simi-priceval">{putThousandsSeparators(price, thousand_separator, decimal_separator, max_number_of_decimals)}</span>
                {!currencyBefore && currency_symbol}
            </span>
        )
    }
...
export const formatPriceLabel = (price, currencyCode = null) => {
    ...
    if (simiGetStoreConfig && simiGetStoreConfig.simiStoreConfig && simiGetStoreConfig.simiStoreConfig.config && simiGetStoreConfig.simiStoreConfig.config.base) {
        const negativeVal = (price < 0)
        price = Math.abs(price)
        const { base } = simiGetStoreConfig.simiStoreConfig.config;
        const { currencies, currency_position, decimal_separator, thousand_separator, max_number_of_decimals } = base;
        let currency_symbol = currencyCode;
        if (!currencyCode || currencyCode === base.currency_code)
            currency_symbol = base.currency_symbol || base.currency_code;
        if (currencies instanceof Array && currencies.length && currencyCode !== base.currency_code && currencies.find(({ value }) => currencyCode === value)) {
        ...
        const putThousandToPrice = putThousandsSeparators(price, thousand_separator, decimal_separator, max_number_of_decimals);

        return (negativeVal ? '-' : '') + (currency_position === "before" ? currency_symbol : '') + putThousandToPrice + (currency_position !== "before" ? currency_symbol : '');
    }
    return ''
}
