...
const authLink = setContext((_, { headers }) => {
    ...
    return {
        headers: {
            ...headers,
            //simi start source from devevelop branch, no need this since 9.0
            ...(storeCode && { 'Store': storeCode }),
            ...(storeCurrency && { 'Content-Currency': storeCurrency }),
            //simi end store/currency switcher
...
const errorLink = onError(({ graphQLErrors, networkError, response, operation }) => {
    ...
    if (response) {
        ...
        //cody return with get cart details because lines below would hide all cart stock errors
        if (operation && operation.operationName && operation.operationName === 'getCartDetails')
            return;
