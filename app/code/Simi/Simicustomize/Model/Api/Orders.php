...
class Orders extends Apiabstract
{
    ...
    public function store()
    {
         ...
         */
        if (
            !$this->simiObjectManager->create('Magento\Customer\Model\Session')->isLoggedIn() &&
            $this->_getQuote()->getPasswordHash() && $quote->getBillingAddress()->getData('customer_password')
        ) {
        ...
        $order = ['invoice_number' => $incrementId,
        ...
        if (!$this->simiObjectManager->create('Magento\Customer\Model\Session')->isLoggedIn() &&
                $this->_getQuote()->getPasswordHash() && isset($parameters['b_address'])) {
            $billingAddress = $parameters['b_address'];
            if (isset($billingAddress->customer_password) && $billingAddress->customer_password) {  
                $checkCustomer = $this->simiObjectManager->create('Simi\Simiconnector\Model\Customer')->getCustomerByEmail($billingAddress->email);
                if (!$checkCustomer->getId()) {
                    $billingAddress->password = $billingAddress->customer_password;
                    $checkCustomer = $this->simiObjectManager->create('Simi\Simiconnector\Model\Customer')->_createCustomer($billingAddress);
                    if ($orderModel) {
                ...
                if ($checkCustomer) {
                    $addressData       = $this->simiObjectManager->get('Simi\Simiconnector\Helper\Address')->convertDataAddress($billingAddress);
                    $addressData['id'] = isset($billingAddress->entity_id) == true ? $billingAddress->entity_id : null;
                    ...
                    $address->setCustomerId($checkCustomer->getId());
                    $address->save();
                }
            }
        }
