...
//        $this->estimateShipping();
    ...
    public function getList($info, $all_ids, $total, $page_size, $from, $is_can_checkout = '1')
    {
        ...
        $validateMinimumAmount = true;
        if (!$this->_getQuote()->validateMinimumAmount()) {
            $validateMinimumAmount = false;
        }
        
        if($this->estimateAddress && $this->estimateShipping && $validateMinimumAmount) {
        ...
        if (!$validateMinimumAmount) {
            $is_can_checkout = '0';
            $result['message'] = [$this->getStoreConfig('sales/minimum_order/error_message')];
        }
