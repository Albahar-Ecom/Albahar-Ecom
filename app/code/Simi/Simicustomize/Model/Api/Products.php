...
class Products extends Apiabstract
{
    ...
    public function show()
    {
        ...
        if (isset($info['brand'])) {
            $brandModel = $this->simiObjectManager->create('Mageplaza\Shopbybrand\Model\BrandFactory')->create()->getBrandCollection(
                null,
                ['main_table.option_id' => ['in' => [$info['brand']]]]
            )->getFirstItem(); 
            $brand = $brandModel->toArray();
            $brand['manufacturer_id'] = $brand['option_id'];
            $brand['name'] = $brand['value'];
            $brand['image'] = $this->simiObjectManager->create('Mageplaza\Shopbybrand\Helper\Data')->getBrandImageUrl($brandModel);        
            $info['brand_model'] = $brand;
        }
